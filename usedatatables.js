(function (factory) {
    ...
    var useDataTable = function (action, param, options) {
        ...
        function draw(settings) {
            if (
                useOptions.className &&
                typeof useOptions.className === 'string' &&
                !$this.hasClass(useOptions.className)
            )
                $this.addClass(useOptions.className);
        ...
        function suma(data) {
            ...
            var len =
                target === 'obj' ? Object.keys(data).length : target === 'arr' ? data.length : 0;

            for (var i = 0; i < len; i++) {
                acumulador +=
                    parseFloat(
                        data[target === 'obj' ? Object.keys(data)[i] : target === 'arr' ? i : i]
                    ) || 0;
            }
            return acumulador;
        }
